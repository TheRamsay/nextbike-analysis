...
uv run nextbike nearest --address "Moravské náměstí, Brno" --reliable
...
uv run nextbike station-departures --address "YOUR_ADDRESS, Brno" --radius-m 600
uv run nextbike citybikes-files --network nextbike-brno
uv run nextbike citybikes-download --network nextbike-brno
uv run nextbike citybikes-summary --network nextbike-brno
uv run nextbike citybikes-area-trend --address "YOUR_ADDRESS, Brno" --radius-m 600 --report-path reports/local_citybikes_area_trend.md
